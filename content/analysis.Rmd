...
ISPo: international_students_max
...
Below is the scatter plot and correlation matrix of ISPo related data to check how the dots are scattered and whether two variables have weak/moderate positive/negative relations. <br>
...
<br> 
Below is the scatter plot and correlation matrix of ISPr related data to check how the dots are scattered and whether two variables have weak/moderate positive/negative relations. <br>
```{r message = FALSE, echo = FALSE, warning = FALSE}
...
<br>
####Boxplots of ISPo and Categorical Variables
```{r message = FALSE, echo = FALSE, warning = FALSE}
library(patchwork)
...
To make the state variable viewable we created the feature "region" using "state" by grouping states into four regions(Midwest, West, South, and Northeast)
...
ggplot(data17_melt, aes(x = variable, y = value, color = factor(region))) + geom_boxplot() + theme_bw() + theme(axis.text.x=element_blank()) + labs(x = "", y = "ISPo")
...
##### IsPr Plot1: Boxplot of ISPr and Research Output
```{r warning = FALSE}
data1 <- clean_data[c("international_students_natlib", "research_output_qs")] %>% drop_na()
data1_melt <- melt(data1, id = "research_output_qs")
p1 <- ggplot(data1_melt, aes(x = variable, y = value, color = factor(research_output_qs))) + geom_boxplot() + theme_bw() +
      theme(axis.text.x=element_blank()) + labs(x = "", y = "ISPr")
p1
```


##### Plot2: Boxplot of ISPr and Research Output
```{r warning = FALSE}
ISPr_Cat <- clean_data[c("international_students_natlib", "research_output_qs","size_qs", "locale_carnegie", "enrprofile2021_carnegie", "type2_carnegie", "type4_carnegie", "world_rank_cat_arwu", "selectivity", "state")] 

data2 <- prev_data[c("international_students_natlib", "size_qs")] %>% drop_na()
data2_melt <- melt(data2, id = "size_qs")
p2 <- ggplot(data1_melt, aes(x = variable, y = value)) + geom_boxplot() + theme_bw() + theme(axis.text.x=element_blank()) + labs(x = "", y = "ISPr")
p2 <- ggplot(data2_melt, aes(x = variable, y = value, color = factor(size_qs))) + geom_boxplot() + theme_bw() +
      theme(axis.text.x=element_blank()) + labs(x = "", y = "ISPr")
p2
```
