...
[Picture source](https://learn.g2.com/hubfs/Imported%20sitepage%20images/1ZB5giUShe0gw9a6L69qAgsd7wKTQ60ZRoJC5Xq3BIXS517sL6i6mnkAN9khqnaIGzE6FASAusRr7w=w1439-h786.png)
...
After trying to gather datasets from multiple sources and some data cleaning work, we decided to make the focus of this project to be: Finding relevant predictors for international student population in U.S. universities and model it to make predictions.

## Breadth and Depth of DA
Our group’s target variables are the international student population (ISPo) and proportion (ISPr) in US universities, and we aim to identify the potential factors that correlate to and predict ISPo and ISPr. 
...
## Early Data Analysis
In our earlier data analysis (see [Blog Post 4](https://clever-unicorn-d34bd5.netlify.app/post/2022-11-04-data-analysis-2-modeling-fitting/)), we examined the relationship between the dependent variable ISPr and the independent variable, an additive indicator reflecting the number of “Alumni of an institution winning Nobel Prizes and Fields Medals” and “Papers indexed in Science Citation Index-Expanded and Social Science Citation Index”. We did this by making predictions with 7 regression models (logistic, quasipolynomial, Gaussian, Poisson, Inverse Gaussian, Gamma) after shuffling and splitting the data with a 70-30 ratio for the train and test sets. By evaluating the AIC and fisher score, we selected the inverse gaussian glm as the best fit of all 7 models. 
...
<br>
Since we are solving a regression problem, our output is a continuous variable. During this phase of data cleaning, we had 35 independent variables concerning every observed college. They are split into 20 continuous variables and 15 categorical variables.    
...
While we have successfully revised the data using sources such as the US Census Bureau and the Univeristy page, we also have took it from sources such as <a href ="https://worldpopulationreview.com/"> World Population Review</a> and <a href ="http://www.city-data.com/"> City Data</a>. We are uncertain how reliable these sources are. In addition, as shown in this [interactive map](https://d63xhp-zeshi-wu.shinyapps.io/SchoolData/), the location of the 441 universities included in our dataset are not nationally representative to a geographic sense. Instead, our data concentrated in the Northeastern region, Florida, Texas, California, and Washington, which are indeed popular states to live and study in for international students.

### Proposed Solution:  
To workaround this, the investigator could webscrape from more reputable sources and limit the number of dataset merges to lessen the missing values and increase the number of observations in other regions.
